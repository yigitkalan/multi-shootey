...
@onready var input: DMPlayerInput = $PlayerInput
@onready var ground_check: RayCast2D = $GroundCheck
@onready var player_health: DMPlayerHealth = $PlayerHealth
@onready var shooter: DMShooter = $Shooter
...
func _on_died():
	if Lobby.is_host():
		...
		_stop_synchronizers()
		var mode = GameManager.current_mode as DMMode
		if not mode:
