class_name DMShooter
extends Node2D

signal shot(knockback_force: Vector2)

@export var shooter_stat: DMShooterStat
@export var collision_exception: Node

@onready var player_input: DMPlayerInput = $"../PlayerInput"
...
func _spawn_bullet(bullet_dir: Vector2, shot_power: float) -> void:
	var bullet: DMBullet = BULLET_SCENE.instantiate()
